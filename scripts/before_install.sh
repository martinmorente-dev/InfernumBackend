...
MYSQL_ROOT_PASSWORD=$(aws ssm get-parameter --name "/infernum/MYSQL_ROOT_PASSWORD" --with-decryption --query "Parameter.Value" --output text --region us-east-1)
MYSQL_USER=$(aws ssm get-parameter --name "/infernum/MYSQL_USER" --with-decryption --query "Parameter.Value" --output text --region us-east-1)
...
mkdir -p /var/www/html/public/Infernum-API/API/setup/
